...
def KaNet(img_dim, resize_factor, output_size, output_activation='softmax', dropout_rate=None, weight_decay=1E-4, noise = 0, verbose=True):
    # Input Layer #
    ...
    output = Convolution2D(8, 2, 2, border_mode='valid')(output)
    # downsampling layer (avg pool)
    output = AveragePooling2D((2, 2), strides=(1, 1))(output)
    output = Activation('relu')(output)
    ...
    for i in range(output_size):
        # Output Subnet
        inter = Dense(128, W_regularizer=l2(weight_decay), b_regularizer=l2(weight_decay))(output)
        inter = PReLU()(inter)
        inter = Dropout(dropout_rate)(inter)
        inter = Dense(1, activation=output_activation)(inter)
        #inter = Dense(1, activation=output_activation, W_regularizer=l2(weight_decay), b_regularizer=l2(weight_decay))(inter)
        outputs.append(inter)
    
    # Output layer #
    # Concat regressor output layers
    output = merge(outputs, mode='concat')
