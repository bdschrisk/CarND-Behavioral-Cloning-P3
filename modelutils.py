...
# Splits sequential data into evenly distributed splits
def sequential_split(X, y, batch_size, split = 0.6):
    train_index, test_index = [], []
    X_copy, y_copy = np.array(X), np.array(y)
    len_test = int((len(y)*(1.0-split))/batch_size)
    ...
    for offset in range(0, len(y), batch_size):
        ...
        if (len(test_index) < len_test) and (count % 2 == 0):
    ...
    (X_train, y_train) = (X_copy[train_index], y_copy[train_index])
    (X_test, y_test) = (X_copy[test_index], y_copy[test_index])
